...
class RobotAndGoalMoveNode(Node):
    def __init__(self):
        super().__init__('robot_and_goal_move_node')
        ...
        self.robot_pose_sub = self.create_subscription(PoseWithCovarianceStamped, '/amcl_pose', self.pose_callback, 1)
    ...
    def pose_callback(self, msg):
        # ロボットの位置情報を取得して表示
        self.robot_current_position = msg.pose.pose.position
        self.robot_current_orientation = msg.pose.pose.orientation
    ...
    def distance_between_follow_marker_and_robot(self):
        ...
        # robot_position
        robot_pose_x = self.robot_current_position.x
        robot_pose_y = self.robot_current_position.y

        ddistance = (robot_pose_y - follow_marker_y)**2 + (robot_pose_x - follow_marker_x)**2
    ...
    def distance_between_follow_marker_and_goal(self):
        ...
        ddistance = (goal_y - follow_marker_y)**2 + (goal_x - follow_marker_x)**2
        return math.sqrt(ddistance)
