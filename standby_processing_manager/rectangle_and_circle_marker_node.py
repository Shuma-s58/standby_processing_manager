...
class RectangleAndCircleMarkerNode(Node):
    def __init__(self):
        super().__init__('rectangle_and_circle_marker_node')
    ...
    def create_rectangle_marker(self):
       ...
       # 位置と姿勢を設定
       self.rectangle_marker.pose.position.x = 1.0
       self.rectangle_marker.pose.position.y = 1.0 - math.cos(self.calculation_radian() + math.radians(90)) * (0.5 / 2.0)
    ...
    def create_circle_marker(self):
       ...
       # 位置と姿勢を設定
       self.circle_marker.pose.position.x = 1.0 - self.rectangle_marker.scale.x / 2.0
       self.circle_marker.pose.position.y = 1.0
    ...
    def calculation_radian(self):
        ...
        # robot position
        robot_x = self.circle_marker.pose.position.x
        robot_y = self.circle_marker.pose.position.y

        # radian
        radian = 0.0

        radian = math.atan2((goal_y - robot_y), (goal_x - robot_x))
...
def main(args=None):
    ...
    node = RectangleAndCircleMarkerNode()
    rclpy.spin(node)
    rclpy.shutdown()
